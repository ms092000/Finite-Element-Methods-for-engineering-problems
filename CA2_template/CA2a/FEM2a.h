...
std::vector<double> FEM<dim>::basis_gradient(unsigned int node, double xi_1, double xi_2){
  ...
  //EDIT
  if(node==0){
    values[0] = -1./2.*(1.-xi_2)/2.;
    values[1] = -1./2.*(1.-xi_1)/2.;
  }
  if(node==1){
    values[0] = 1./2.*(1.-xi_2)/2.;
    values[1] = -1./2.*(1.+xi_1)/2.;
...
void FEM<dim>::define_boundary_conds(){
  ...
  for(int i=0;i<totalNodes;i++){
    if(nodeLocation[i][1] == 0.){
      boundary_values[i] = 300*(1+c*nodeLocation[i][0]);
    }
    if(nodeLocation[i][1] == 0.08){
      boundary_values[i] = 310*(1+c_hat*pow(nodeLocation[i][0], 2));
    }
  }
}
	  ...
	  for(unsigned int B=0; B<dofs_per_elem; B++){
		...
		for(unsigned int I=0;I<dim;I++){
		  for(unsigned int J=0;J<dim;J++){
		    ...
		    Klocal[A][B] += basis_gradient(A, quad_points[q1], quad_points[q2])[I]*invJacob[I][i]*kappa[i][j]*invJacob[J][j]*basis_gradient(B, quad_points[q1], quad_points[q2])[J]*detJ*quad_weight[q1]*quad_weight[q2];
		  }
		}
	      }
	    }
