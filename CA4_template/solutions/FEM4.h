...
void FEM<dim>::assemble_system(){
  ...
  double		    rho = 3.8151*1e6;                            //EDIT - specify the specific heat per unit volume
...
void FEM<dim>::solve_trans(){
  ...
  for(unsigned int t_step=1; t_step<3001; t_step++){
    ...
    //EDIT
    D_trans.add((1-alpha)*delta_t, V_trans);
    ...
    //EDIT
    D_tilde.add(alpha*delta_t, V_trans);
