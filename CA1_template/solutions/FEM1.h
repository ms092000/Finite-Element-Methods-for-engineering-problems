...
double FEM<dim>::basis_function(unsigned int node, double xi){
  ...
  double value = 1.;   //Store the value of basis function in this variable. 
  
  if(basisFunctionOrder==1)
    {
      double N0 = xi_at_node(0);
      double N1 = xi_at_node(1);
      if(node==0){
	value = (xi-N1)/(N0-N1);
      }
      if(node==1){
  ...
  if(basisFunctionOrder==2)
    {
      double N0 = xi_at_node(0);
      double N1 = xi_at_node(1);
      double N2 = xi_at_node(2);
      if(node==0){
	value = ((xi-N1)*(xi-N2))/((N0-N1)*(N0-N2));
      }
      if(node==1){
	value = ((xi-N0)*(xi-N2))/((N1-N0)*(N1-N2));
      }
      if(node==2){
	value = ((xi-N0)*(xi-N1))/((N2-N0)*(N2-N1));
      }
    }
  ...
  if(basisFunctionOrder==3)
    {
      ...
      double N1 = xi_at_node(1);
      double N2 = xi_at_node(2);
      double N3 = xi_at_node(3);
      if(node==0){
	value = ((xi-N1)*(xi-N2)*(xi-N3))/((N0-N1)*(N0-N2)*(N0-N3));
      }
      if(node==1){
	value = ((xi-N0)*(xi-N2)*(xi-N3))/((N1-N0)*(N1-N2)*(N1-N3));
      }
      if(node==2){
	value = ((xi-N0)*(xi-N1)*(xi-N3))/((N2-N0)*(N2-N1)*(N2-N3));
      }
      if(node==3){
	value = ((xi-N0)*(xi-N1)*(xi-N2))/((N3-N0)*(N3-N1)*(N3-N2));
      }
    }
  ...
  if(basisFunctionOrder==2)
    {
      ...
      double N2 = xi_at_node(2);
      if(node==0){
	value = ((xi-N2)+(xi-N1))/((N0-N1)*(N0-N2));
      }
      if(node==1){
	value = ((xi-N2)+(xi-N0))/((N1-N0)*(N1-N2));
      }
      if(node==2){
	value = ((xi-N1)+(xi-N0))/((N2-N0)*(N2-N1));
      }
    }
  ...
  if(basisFunctionOrder==3)
    {
      ...
      double N3 = xi_at_node(3);
      if(node==0){
	value = ((xi-N2)*(xi-N3)+(xi-N1)*(xi-N3)+(xi-N1)*(xi-N2))/((N0-N1)*(N0-N2)*(N0-N3));
      }
      if(node==1){
	value = ((xi-N2)*(xi-N3)+(xi-N0)*(xi-N2)+(xi-N0)*(xi-N3))/((N1-N0)*(N1-N2)*(N1-N3));
      }
      if(node==2){
	value = ((xi-N1)*(xi-N3)+(xi-N0)*(xi-N3)+(xi-N0)*(xi-N1))/((N2-N0)*(N2-N1)*(N2-N3));
      }
      if(node==3){
	value = ((xi-N1)*(xi-N2)+(xi-N0)*(xi-N2)+(xi-N0)*(xi-N1))/((N3-N0)*(N3-N1)*(N3-N2));
      }
    }
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
	...
	//EDIT - Define Flocal.
	double fbar = pow(10, 11);
	double Ar = pow(10, -4);
	f = fbar * x;
	Flocal(A) += (f*Ar*h_e/2)*basis_function(A, quad_points[q])*quad_weight[q];
      }
    }
	...
	for(unsigned int q=0; q<quadRule; q++){
	  //EDIT - Define Klocal.
	  Klocal(A, B) += (2*E*Ar/h_e)*basis_gradient(A, quad_points[q])*basis_gradient(B, quad_points[q])*quad_weight[q];
	}
      }
    }
...
double FEM<dim>::l2norm_of_error(){
  ...
  for (;elem!=endc; ++elem){
	...
	u_h += D[local_dof_indices[B]]*basis_function(B,quad_points[q]);
      ...
      if(prob == 1){
	  u_exact = (-pow(x, 3) + 0.07*x)/6.; 
	}
      if(prob == 2){
	  u_exact = (-pow(x, 3) + 0.63*x)/6.;
	}
      double diff = u_exact - u_h;
      l2norm += pow(diff, 2)*h_e/2.*quad_weight[q];
    }
  }
