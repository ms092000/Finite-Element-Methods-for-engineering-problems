...
double FEM<dim>::basis_function(unsigned int node, double xi_1, double xi_2, double xi_3){
  ...
  if(node==0){
    value*=((1-xi_1)/2.)*((1-xi_2)/2.)*((1-xi_3)/2.);
  }
  if(node==1){
    value*=((1+xi_1)/2.)*((1-xi_2)/2.)*((1-xi_3)/2.);
  }
...
std::vector<double> FEM<dim>::basis_gradient(unsigned int node, double xi_1, double xi_2, double xi_3){
  ...
  //EDIT
  if(node==0){
    values[0] = -1./2.*(1.-xi_2)/2.*(1.-xi_3)/2.;
    values[1] = -1./2.*(1.-xi_1)/2.*(1.-xi_3)/2.;
    values[2] = -1./2.*(1.-xi_1)/2.*(1.-xi_2)/2.;
  }
  if(node==1){
    values[0] = 1./2.*(1.-xi_2)/2.*(1.-xi_3)/2.;
    values[1] = -1./2.*(1.+xi_1)/2.*(1.-xi_3)/2.;
...
void FEM<dim>::define_boundary_conds(){
  ...
  for(int i=0; i<totalNodes; i++){
    if(nodeLocation[i][0]==0.){
      boundary_values[i] = 300*(1+c*(nodeLocation[i][1]+nodeLocation[i][2])); 
    }
    if(nodeLocation[i][0]==0.04){
      boundary_values[i] = 310*(1+c*(nodeLocation[i][1]+nodeLocation[i][2]));
    }
  }
}
...
void FEM<dim>::setup_system(){
  ...
  quad_points[2] = -1./3.*sqrt(5.-2.*sqrt(10./7.)); //EDIT
  ...
  quad_weight[2] = (322.+13.*sqrt(70.))/900.; //EDIT
		...
		for(unsigned int j=0;j<dim;j++){
		  for(unsigned int I=0;I<dim;I++){
		    for(unsigned int J=0;J<dim;J++){
		      ...
		      Klocal[A][B] += basis_gradient(A, quad_points[q1], quad_points[q2], quad_points[3])[I]
			*invJacob[I][i]*kappa[i][j]*invJacob[J][j]
